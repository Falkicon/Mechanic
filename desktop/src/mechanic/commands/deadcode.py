...
def is_library_under_development(addon_path: Path, addon_name: str) -> bool:
    ...
    for sibling in addon_parent.iterdir():
        ...
        libs_json_paths = [sibling / "Libs" / "libs.json", sibling / "libs.json"]
                    ...
                    if isinstance(include, dict):
                        for lib_name, lib_config in include.items():
                            if (
                                ...
                                if (
                                    lib_name == addon_name
                                    or lib_name.lower() == addon_name.lower()
                                ):
                                    return True
            except Exception:
                pass
...
def find_unused_libraries(
            ...
            with open(libs_json) as f:
                ...
                if "include" in config:
                    ...
                    for lib_folder in libs_path.iterdir():
                        if (
                            lib_folder.is_dir()
                            and lib_folder.name not in config["exclude"]
                        ):
    ...
    for lib_name in declared_libs:
        ...
        is_used = any(
            lib_base.lower() in used_lib.lower() or lib_name.lower() in used_lib.lower()
...
def find_unused_locale_strings(
    addon_path: Path, analyzer: LuaAnalyzer
) -> List[DeadCodeIssue]:
...
def find_unreachable_code(
    addon_path: Path, files_to_analyze: List[Path]
) -> List[DeadCodeIssue]:
...
def find_stale_event_handlers(
    addon_path: Path, analyzer: LuaAnalyzer, files_content: Dict[Path, str]
) -> List[DeadCodeIssue]:
...
def analyze_addon(
    ...
    if DeadCodeCategory.STALE_EVENT.value in categories:
        all_issues.extend(
            find_stale_event_handlers(addon_path, analyzer, files_content)
        )
