...
def validate_toc(addon_path: Path, addon_name: str) -> ValidationResult:
    ...
    if not interface_versions:
        ...
        if not has_valid:
            result.errors.append(f"Interface version outdated: {', '.join(interface_versions)}. Should include 120001 or 120000")
            result.valid = False
        else:
    ...
    if 'SavedVariables' in metadata:
        ...
        if not sv_name.startswith(addon_name) and not sv_name.startswith(normalized):
            result.warnings.append(f"SavedVariables '{sv_name}' doesn't match addon name '{addon_name}'")
    ...
    if locales_path.exists():
        ...
        if not enus_path.exists():
            result.warnings.append("Locales folder exists but enUS.lua (baseline) is missing")
        else:
            has_enus_in_toc = any('Locales' in f['path'] and 'enUS' in f['path'] for f in files)
            if not has_enus_in_toc:
                result.warnings.append("enUS.lua exists but is not referenced in the .toc file")
...
def register_commands(server):
    ...
    async def validate_addon(input: AddonInput, context: Any = None) -> CommandResult[ValidationResult]:
    ...
    async def lint_addon(input: LintInput, context: Any = None) -> CommandResult[LintResult]:
        import subprocess
        ...
        try:
            result = subprocess.run(
                [str(luacheck_path), str(addon_path), "--formatter", "plain", "--codes", "--no-color"],
                capture_output=True,
                text=True,
                timeout=60
            )
    ...
    class FormatInput(BaseModel):
        ...
        path: Optional[str] = Field(None, description="Override path to addon folder")
        check: bool = Field(False, description="Only check formatting, don't modify files")
    ...
    async def format_addon(input: FormatInput, context: Any = None) -> CommandResult[FormatResult]:
        import subprocess
    ...
    async def test_addon(input: TestInput, context: Any = None) -> CommandResult[TestResult]:
        import subprocess
        ...
        try:
            # Busted JSON output
            output = json_module.loads(result.stdout) if result.stdout.strip().startswith('{') else {}
            for test in output.get('successes', []):
                tests.append(TestCase(name=test.get('name', 'Unknown'), passed=True, duration=test.get('duration', 0)))
                passed_count += 1
            for test in output.get('failures', []):
                tests.append(TestCase(name=test.get('name', 'Unknown'), passed=False, error=test.get('message', '')))
                failed_count += 1
        except Exception:
    ...
    async def scan_deprecations(input: DeprecationInput, context: Any = None) -> CommandResult[DeprecationResult]:
        addon_path = find_addon_path(input.addon, input.path)
        if not addon_path:
            return error(
                code="ADDON_NOT_FOUND",
