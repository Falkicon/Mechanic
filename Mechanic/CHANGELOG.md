...
- **Combat Lockdown Documentation**: Added `mechanic:ignore-combat` suppression comments to intentional combat-safe UI operations in Inspect, MainFrame, and Utils modules.

### Fixed
- **Deprecated API**: Fixed `GetSpecializationInfo` to use modern `C_SpecializationInfo.GetSpecializationInfo` pattern in Utils.lua.
- **MCP Stability**: Removed `git.commit`, `git.tag`, and `release.all` from MCP server registration to prevent async subprocess hangs. Use CLI directly for git operations.
