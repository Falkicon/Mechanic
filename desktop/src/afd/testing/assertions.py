...
def assert_error(
    ...
    if expected_code is not None and error.code != expected_code:
        failure_msg = (
            message or f"Expected error code '{expected_code}' but got '{error.code}'"
        )
        raise AssertionError(failure_msg)
...
def assert_has_confidence(
    ...
    """
    confidence = (
        result.get("confidence") if isinstance(result, dict) else result.confidence
    )
...
def assert_has_reasoning(
    ...
    """
    reasoning = (
        result.get("reasoning") if isinstance(result, dict) else result.reasoning
    )
...
def assert_has_alternatives(
    ...
    """
    alternatives = (
        result.get("alternatives") if isinstance(result, dict) else result.alternatives
    )
